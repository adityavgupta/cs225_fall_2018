...
PNG createSpotlight(PNG image, int centerX, int centerY) {
  for(unsigned int x = 0; x < image.width(); x++){
    for(unsigned int y = 0; y < image.height(); y++){
      ...
      HSLAPixel & pixel = image.getPixel(x, y);
      double euclidean = std::sqrt(((centerX - x)*(centerX - x)) + ((centerY - y)*(centerY - y)));
      if(euclidean < 160){
          pixel.l *= 1 - (0.005*euclidean);
      }
      else{
          pixel.l *= 0.2;
      }
...
PNG watermark(PNG firstImage, PNG secondImage) {
    int width_limit = secondImage.width();
    if(firstImage.width() < secondImage.width()){
        width_limit = firstImage.width(); 
    }

    int height_limit = secondImage.height();
    if(firstImage.height() < secondImage.height()){
        height_limit = firstImage.height();
    }
