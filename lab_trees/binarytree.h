...
class BinaryTree
{
    ...
    private:
		...
		/*
		 * Private helper function for the public member function mirror
		 */
		void mirror(Node*& subRoot);

		//Private helper functions for IsOrdered recursive
		bool isOrderedR(Node* subRoot) const;
		T maxValLeft(Node* subRoot) const;
		Node* maxVal(Node* n) const;

		T minValRight(Node* subRoot) const;
		Node* minVal(Node* n) const;

		//Private helper function for path 
		void pathHelper(Node* subRoot, vector<vector<T> >& allpaths, vector<T> path) const ;
