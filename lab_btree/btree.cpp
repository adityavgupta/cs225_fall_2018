...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
    ...
    if (!subroot->elements.empty() && first_larger_idx < subroot->elements.size() && first_larger_idx >= 0) {
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    else if (!subroot->elements.empty() && first_larger_idx < subroot->elements.size() && subroot->elements[first_larger_idx].key == pair.key){
        return;
    }
    else {
