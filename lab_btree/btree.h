 ...
 */
 ...
 size_t index(const std::vector<T>& elem, const C& val, size_t low, size_t high) {
     size_t mid_index = (low + high)/2;
     if (val == elem[mid_index])
         return mid_index;
     if (val > elem[mid_index] && val < elem[mid_index + 1])
         return mid_index + 1;
     
     if (val < elem[mid_index])
         return index(elem, val, low, mid_index-1);
     else
         return index(elem, val,mid_index+1, high);
 }
 /**
  * Generalized function for finding the insertion index of a given element
  * into a given sorted vector.
 ...
 size_t insertion_idx(const std::vector<T>& elements, const C& val)
 {
    ...
    return index(elements, val, 0, elements.size() - 1);
