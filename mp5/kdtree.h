...
class KDTree
{
  ...
  private:
    ...
    /** a vector off which the KDtree will be made
    * Will modify this vector to create the KDTree.
    */
	...
	// helper functions for the Ctor starting here
	
    /**
     * a partition helper function for quick sort used in ctor
     * parameters: start point, an end point, the pivot index wrt which partition is done, dimension.
     */

	  int partition(int start, int end, int pivotIndex, int dimension);

    /**
	 * quick select function. Uses partition to achieve its functionality.
	 */

	  void quickSelect(int start, int end, int k, int dimension);

	  /**
	   * build tree function. builds the tree from the root.
	   */

	  KDTreeNode* buildTree(int start, int end, int dimension);

	  //helper function for nearest neighbor
	  void nnsHelper(KDTreeNode* croot, const Point<Dim>& query, Point<Dim>& c, double &dist, int dimension) const;
	  
	  //helper function to check whether a query point is within range or not.
	  bool within(Point<Dim> query, Point<Dim> current, double dist, int dimension) const;

	  //helper funciton for the destructor
