                            ...
                            queue<TreeNode*>& mergeQueue)
{
     ...
     */
	//create a temp TreeNode pointer
	TreeNode* temp;
	//check if single queue is empty
	if(singleQueue.empty()) {
		//if both empty return NULL;
		if(mergeQueue.empty()) {
			return NULL;
		} 
		//otherwise the smallest value is in the mergeQueue.
		else {
	...
	//check if the mergeQueue is empty
	else if(mergeQueue.empty()) {
		//return NULL if both empty
		if(singleQueue.empty()) 
			return NULL;
		//otherwise smallest elemt is the first element of singleQueue.
		else {
	...
	//if both exist
		...
		//check which of the two first elements in the respective queues is larger
		if(singleQueue.front()->freq.getFrequency() > mergeQueue.front()->freq.getFrequency()) {
			//if singleQueue has larger first elem temp is assigned mergeQueue's front value
			temp = mergeQueue.front();
			mergeQueue.pop();
			return temp;
		}
			//otherwise singleQueue's front value is assigned to temp
		else {
			temp = singleQueue.front();
			singleQueue.pop();
			return temp;
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
	...
	while(!((singleQueue.empty() && mergeQueue.size() == 1) || (singleQueue.size() == 1 && mergeQueue.empty()))) {
		TreeNode* smaller = removeSmallest(singleQueue, mergeQueue);
		TreeNode* larger = removeSmallest(singleQueue, mergeQueue);
		TreeNode* parent = new TreeNode(smaller->freq.getFrequency() + larger->freq.getFrequency());
		parent->left = smaller;
		parent->right = larger;
