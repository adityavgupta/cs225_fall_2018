...
#include "letter.h"
#include "room.h"
#include <string> //changed this -Adi
/**
...
class Allocator
{
  public:
     ...
     */
    Allocator(const std::string& studentFile, const std::string& roomFile); //added std:: in thid line -Adi
    ...
    ~Allocator();//added destructor -Adi
  ...
  private:
     ...
     */
    void loadStudents(const std::string& file); //added std:: -Adi

    /**
     * Reads rooms from the rooms file and sets their capacities.
     * @param file The file to read the room list from
     */
    void loadRooms(const std::string& file); //added std:: -Adi
